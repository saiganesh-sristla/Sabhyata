...
const SpecialEvent = () => {
  ...
  useEffect(() => {
    console.log("SpecialEvent id:", id);
    const fetchEvent = async () => {
      try {
        ...
        console.log("SpecialEvent API Response:", data);
        if (data.success) {
          setEvent(data.data);
        } else {
  ...
  const handleShare = async () => {
    const shareData = {
      title: event.name,
      text: "Join me in exploring this amazing event! Discover the history and culture at the Red Fort Light & Sound Show.",
  ...
  return (
    <div
      className="min-h-screen bg-cover bg-center md:bg-no-repeat bg-fixed text-foreground"
      style={{
        backgroundImage: `url('/jai-hind-bg.png')`,
    ...
    >
      <div className="relative z-10">
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="lg:w-2/3 w-full space-y-6">
              <div className="text-black px-4 md:px-6 flex justify-between items-center">
                ...
                <button
                  onClick={handleShare}
                  className="flex items-center bg-[#FBF1E4]/90 px-3 md:px-4 py-3 md:py-2 rounded-lg space-x-2 text-gray-900 hover:text-[#5C4033]"
                >
              ...
              <Card className="bg-[#FBF1E4]/90">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {youShouldKnowItems.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-start space-x-3 p-3 bg-[#982A3D]/10 rounded-xl"
                    >
                      <div className="p-2">
                  ...
                  {[
                    {
                      ...
                      desc: "State-of-the-art technology with projection mapping",
                    },
                    {
                      icon: Book,
                      title: "Rich History",
                      desc: "Journey through centuries of Indian heritage",
                    },
                    {
                      icon: Bell,
                      title: "Cultural Insight",
                      desc: "UNESCO World Heritage Site experience",
                    },
                  ].map((item, index) => (
                    <div key={index} className="text-center p-4">
                      <div className="w-14 h-14 bg-[#982A3D]/10 rounded-full flex items-center justify-center mx-auto mb-3">
        ...
        <div className="w-full mt-12">
          <img
            src="/monument-bg.png"
            alt="Monument Background"
            className="w-full h-auto mb-16 md:mb-0 object-cover opacity-50"
